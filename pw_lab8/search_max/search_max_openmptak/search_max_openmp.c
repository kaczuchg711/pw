...
double search_max_openmp_task(double *A, int p, int k)
{
    ...
    double a_max = A[p];
    double aMaxForTask =a_max;
#pragma omp parallel default(none) shared(a_max) private(aMaxForTask) firstprivate(A, p, k)
    {
...
// uz
#pragma omp task shared(a_max) firstprivate(A, p_task, k_task,aMaxForTask) default(none)
                {
                    for (int i = ++p_task; i <= k_task; ++i)
                        if (aMaxForTask < *(A + i)) aMaxForTask = *(A + i);

#pragma omp critical
                        if (aMaxForTask > a_max) a_max = aMaxForTask;
//
                } // end loop over tasks
            } // end single region
        } // end parallel region
